...
void igmp_init() {
	...
	val = PortRead(0x0, 4);
	// write("0x0 0x4: 0x%04X\n", val);
	val = val & 0xFFF3;
	PortWrite(0x0, 4, val | 0x040C);
	// write("0x0 0x4: 0x%04X --> 0x%04X\n", val, PortRead(0x0, 4));

	val = PortRead(0x1, 4);
	// write("0x1 0x4: 0x%04X\n", val);
	val = val & 0xFFF3;
	PortWrite(0x1, 4, val | 0x040C);
	// write("0x1 0x4: 0x%04X\n", PortRead(0x1, 4));

	val = PortRead(0x2, 4);
	// write("0x2 0x4: 0x%04X\n", val);
	val = val & 0xFFF3;
	PortWrite(0x2,4, val | 0x040C);
	// write("0x2 0x4: 0x%04X\n", PortRead(0x2, 4));

	val = PortRead(0x3, 4);
	// write("0x3 0x4: 0x%04X\n", val);
	val = val & 0xFFF3;
	PortWrite(0x3,4, val | 0x040C);
	// write("0x3 0x4: 0x%04X\n", PortRead(0x3, 4));

	val = PortRead(0x4, 4);
	// write("0x4 0x4: 0x%04X\n", val);
	val = val & 0xFFF3;
	PortWrite(0x4, 4, val | 0x040C);
	// write("0x4 0x4: 0x%04X\n", PortRead(0x4, 4));
